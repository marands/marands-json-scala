...
class AkkaHttpSupport {
  ...
  private val sourceJsonStringMarshaller =
  ...
  implicit def unmarshaller[A](implicit
    ...
    typeOf[A] match {
      case t if t =:= typeOf[String] =>
        jsonStringUnmarshaller.map(a => a.asInstanceOf[A])
      case _ =>
        jsonStringUnmarshaller.map { data =>
          objectMapper.readValue(data, typeReference[A])
        }
    }
  ...
  implicit def sourceMarshaller[A](implicit
    ...
    sourceJsonStringMarshaller.compose(jsonSource[A])
}
